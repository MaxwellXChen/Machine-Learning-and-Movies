...
LASSO selection was run on the preprocessed data with the goal of identifying important features to include in the data we run through our model to predict IMDb score. The input data for the LASSO selection model was first normalized, as were the output values for IMDb score. As shown in the figure below, year and duration were the two most important features as selected by SFM with an importance threshold specifying a minimum coefficient of .2. When using sequential feature selection with lasso regression with 3 features to duration, language, and budget were selected for both forward and backward selection.
![LASSOCVIMDb](https://user-images.githubusercontent.com/72351531/145162226-78846816-c6ae-4765-95c8-36feae98281b.PNG)

LASSO selection was run on the preprocessed data with the goal of identifying important features to include in the data we run through our model to predict worldwide gross. The input data for the LASSO selection model was first normalized, as were the output values for gross income. As shown in the figure below, budget and number of votes were the two most important features as selected by SFM with an importance threshold specifying a minimum coefficient of .2. This makes sense rationally as it follows that a movie that grosses well also generates a lot of interest, which leads to votes on review platforms. It also follows that movies with larger budgets usually gross more money. When using sequential feature selection with lasso regression with 3 features to select, votes, budget, and metascore were selected for both forward and backward selection.
